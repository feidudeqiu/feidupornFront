<template>
    <div>
        ...
        <Drawer :closable="true" :draggable="true" @on-close="noteFlag = false" :scrollable="true" :width="840" :mask="false" placement="right" title="笔记列表" v-model="noteFlag">
           <Table :width="802" :max-height="windowHeight-90" :columns="noteColumns" :data="noteData.filter(data => !search || data.title.toLowerCase().includes(search.toLowerCase()) || data.no.toLowerCase().includes(search.toLowerCase()))">
                <template slot-scope="{ row, index }" slot="action">
        ...
        <div class="content">
            <div class="flex-column-center" style="margin-top:30px;">
                <div style="width:100%;height:30px;border-bottom:1px solid #ddd;margin-bottom:45px;">
                    <span style="color:black;background:white;font-size:38px;line-height:58px;padding:0 20px;">创建笔记集</span>
                </div>
...
import {Table, Drawer, Input, Icon, RadioGroup, Radio} from "view-design"
export default {
    name: "AddNoteSet",
    components: {
        "porn-head": Head,
    ...
    data () {
        return {
            ...
            title: '',
            noteSetType: 'public',
            image: 'http://127.0.0.1:8080/global/44d849ac-47f5-11ea-8755-525400fecd39.jpeg',
            setColumns: [
                {
                    title: '序号',
                    slot: 'index',
    ...
    created () {
        var that = this;
        that.axios.get('/api/note/get-note')
            .then(function (res) {
                console.info(res);
